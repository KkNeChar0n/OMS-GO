    ...
    <div class="filter-form">
      <div class="filter-row">
        <div class="filter-item">
          ...
          <input type="number" id="copiedFlowIdFilter" v-model="filters.id" placeholder="请输入ID">
        </div>
        <div class="filter-item">
          <label for="copiedFlowFlowIdFilter">审批流ID</label>
          <input type="number" id="copiedFlowFlowIdFilter" v-model="filters.approval_flow_id" placeholder="请输入审批流ID">
        </div>
        <div class="filter-item">
          <label for="copiedFlowTypeFilter">审批流类型</label>
          <select id="copiedFlowTypeFilter" v-model="filters.approval_flow_type_id">
...
import { getCopiedFlows, getApprovalFlowDetail } from '../../api/approval'
  ...
  setup(props) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await getCopiedFlows(params)
        flows.value = response.data.data.copied_flows || []
      } catch (error) {
    ...
    const openApprovalDetail = async (flow) => {
      try {
        const response = await getApprovalFlowDetail(flow.approval_flow_id)
