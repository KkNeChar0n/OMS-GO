<template>
  <div class="refund-childorders-container">
    ...
    <div class="filter-form">
      <div class="filter-row">
        ...
        <div class="filter-item">
          <label for="goodsIdFilter">商品ID</label>
          <input
            type="number"
            id="goodsIdFilter"
            v-model="filters.goods_id"
            placeholder="请输入商品ID"
          >
        </div>
        <div class="filter-item">
...
import { getRefundChildOrders } from '@/api/order';
...
export default {
  ...
  setup() {
    ...
    // 获取子退费订单列表
    const fetchRefundChildOrders = async () => {
      loading.value = true;
      try {
        const params = {};
        ...
        if (filters.value.goods_id) params.goods_id = filters.value.goods_id;
        if (filters.value.status !== '') params.status = filters.value.status;

        const response = await getRefundChildOrders(params);
        refundChildOrders.value = response.data.refund_childorders || [];
      } catch (error) {
        console.error('获取子退费订单列表失败:', error);
    ...
    // 搜索
    const searchRefundChildOrders = () => {
      currentPage.value = 1;
      fetchRefundChildOrders();
    };
    ...
    const paginatedData = computed(() => {
      ...
      const end = start + 10;
      return refundChildOrders.value.slice(start, end);
    });

    // 总页数
    const totalPages = computed(() => {
      return Math.ceil(refundChildOrders.value.length / 10);
    });
